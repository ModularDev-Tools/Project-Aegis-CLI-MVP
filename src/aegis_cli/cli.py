...
@click.option("--language", "-l", type=click.Choice(get_supported_languages(), case_sensitive=False), help="Force a specific language, ignoring detection.")
def generate_command(project_path, output, verbose, dry_run, language):
    ...
    if not language:
        raise click.ClickException(
            "Could not detect a supported language. Use the --language flag to specify one."
        )
