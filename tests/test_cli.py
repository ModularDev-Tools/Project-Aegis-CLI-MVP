...
def test_generate_force_language(runner, python_project):
    """Test that --language overrides detection."""
    # This is a python project, but we will force javascript generation
    result = runner.invoke(
        cli, ["generate", str(python_project), "--language", "javascript"]
    )
    assert result.exit_code == 0

    guide_path = python_project / "security/SecureCodingGuide.md"
    assert guide_path.exists()
    content = guide_path.read_text()
    assert "Secure Coding Guide: Javascript" in content
    assert "Secure Coding Guide: Python" not in content


def test_generate_force_invalid_language(runner, empty_project):
    """Test that forcing an invalid language fails."""
    result = runner.invoke(cli, ["generate", str(empty_project), "--language", "cobol"])
    assert result.exit_code != 0  # Should be 2 for invalid choice from Click
    assert "Invalid value for '--language' / '-l'" in result.output
